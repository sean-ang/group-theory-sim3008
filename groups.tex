...
\subsection{Groups}
	\begin{define}\label{definition-groups}
		(Definition of groups) Let $G$ be a set and $\cdot$ an operation on $G$
		\begin{itemize}
...
\subsection{Homomorphisms}
	\begin{define}
		Let $G$ and $H$ be groups. A function $f:G \to H$ is a homomorphism if $f(ab)=f(a)f(b)$ for all $a,b\in G$. If $f$ is surjective then we call $f$ an epimorphism. If $f$ is bijective we call $f$ an isomorphism.
	\end{define}
